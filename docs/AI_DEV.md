...
.
├── docs/                      # 项目文档
│   ├── AI_DEV.md              # AI 辅助开发规范 (非常棒的设计，继续保持)
|   |── CONTEXT_FULL.md        # 上下文,每次完成任务AI都会自动更新
├── supabase/                  # Supabase 全栈后端服务
│   ├── migrations/            # 数据库版本控制 (SQL migrations)
│   ├── functions/             # 边缘函数 (Deno Edge Functions)
│   ├── seed.sql               # 开发环境的假数据填充脚本
│   └── config.toml            # Supabase 本地 CLI 配置
├── src/
│   ├── api/                   # 外部 API 封装 (如果除了 Supabase 还有支付/短信等接口)
│   ├── assets/                # 静态资源
│   │   ├── images/            # 图片、图标
│   ├── components/            # 局部复用组件 (你之前遗漏的关键目录)
│   │   ├── common/            # 基础 UI (Button, Modal, Input 等)
│   │   ├── business/          # 业务组件 (如 UserAvatar, PaymentCard)
│   │   └── icons/             # SVG 图标组件
│   ├── composables/           # 组合式函数 (Vue3 核心：复用状态逻辑)
│   │   ├── useAuth.ts         # 封装登录/注册/会话管理
│   │   └── useProfile.ts      # 封装用户资料获取与更新
│   ├── layouts/               # 全局布局组件
│   │   ├── DefaultLayout.vue  # 登录后的带侧边栏布局
│   │   └── AuthLayout.vue     # 登录/注册页面的空白布局
│   ├── lib/                   # 第三方库的实例化与配置
│   │   ├── supabase.ts        # Supabase 客户端实例
│   │   └── dayjs.ts           # 日期处理库配置等
│   ├── pages/                 # 路由页面 (配合 layouts 使用)
│   │   ├── dashboard/         # 按模块划分文件夹
│   │   │   └── index.vue      
│   │   └── auth/
│   │       └── login.vue
│   ├── router/                # 路由配置
│   │   ├── index.ts           # 路由表定义
│   │   └── guards.ts          # 路由守卫 (非常重要！在这里拦截未登录用户)
│   ├── stores/                # 状态管理 (Pinia)
│   │   └── userStore.ts       # 缓存用户状态、权限
│   ├── types/                 # TypeScript 全局类型
│   │   ├── index.d.ts         # 业务自定义类型
│   │   └── database.types.ts  # Supabase 自动生成的数据库 TS 类型
│   ├── utils/                 # 纯工具函数 (无副作用)
│   │   ├── formatters.ts      # 格式化时间、货币
│   │   └── validators.ts      # 表单正则校验
│   ├── App.vue                # Vue 根组件
│   └── main.ts                # 项目入口文件
├── tests/                     # 测试代码
