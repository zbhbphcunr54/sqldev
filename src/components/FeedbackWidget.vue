...
const category = ref<FeedbackRequest['category']>('idea')
const content = ref('')
...
<template>
  <aside class="fixed bottom-6 right-6 z-50">
    ...
    <div v-if="open" class="mt-3 w-[320px] rounded-card border border-border bg-panel p-4 shadow-soft">
      ...
      <div class="mt-3 flex justify-end gap-2">
        ...
        <button class="btn-primary px-3 py-2 text-sm" :disabled="!canSubmit || loading" @click="handleSubmit">
          {{ loading ? '提交中...' : '提交建议' }}
        </button>
      </div>
    </div>
