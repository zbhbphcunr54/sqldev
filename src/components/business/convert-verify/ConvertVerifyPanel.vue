...
<template>
  <div class="border border-border rounded-lg bg-panel overflow-hidden">
    <!-- Header -->
    <div class="px-4 py-3 border-b border-border bg-panel2 flex items-center justify-between">
      <div class="flex items-center gap-3">
        <span class="font-medium text-text">AI 校验</span>
        <span
          v-if="currentQuota"
          class="text-xs px-2 py-1 rounded"
          :class="
            currentQuota.remaining > 0 ? 'bg-successBg text-success' : 'bg-dangerBg text-danger'
          "
        >
      ...
      <button
        ...
        :class="[
          ...
          canVerify && !loading
            ...
            : 'bg-panel3 text-muted cursor-not-allowed'
        ]"
        @click="handleVerify"
      >
        {{ loading ? '校验中...' : '开始校验' }}
    ...
    <div class="p-4">
      <div v-if="loading" class="flex items-center justify-center py-12">
        <div class="animate-spin h-8 w-8 border-2 border-brand-600 border-t-transparent rounded-full"></div>
        <span class="ml-3 text-subtle">正在分析 SQL...</span>
      </div>

      <div v-else-if="errorMessage" class="text-center py-8">
        <p class="text-danger mb-2">校验失败</p>
        <p class="text-subtle text-sm">{{ errorMessage }}</p>
        <button
          class="mt-3 px-4 py-1.5 bg-brand-600 text-white rounded text-sm hover:bg-brand-700"
          @click="handleVerify"
        >
          重试
        </button>
      ...
      <div v-else-if="result?.ok && result.overallScore !== undefined" class="space-y-6">
        ...
        <div class="border border-border rounded-lg overflow-hidden">
          <div class="flex border-b border-border bg-panel2">
            <button class="px-4 py-2 text-sm font-medium text-brand-600 border-b-2 border-brand-600">
              全部问题 ({{
        ...
        <div v-if="result.suggestions && result.suggestions.length > 0">
          <h3 class="text-sm font-medium text-text mb-3">修改建议</h3>
