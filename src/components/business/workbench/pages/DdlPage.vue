  ...
  <div class="ddl-page">
    ...
    <section class="workspace">
      ...
      <article class="panel">
        <div class="panel-head">
          <div class="panel-head-left">
            <span class="panel-dot" :class="store.sourceDb"></span>
            <h2 class="panel-title">{{ store.sourceLabel }} 输入</h2>
          </div>
          <span class="panel-paste-hint">Ctrl / ⌘ + Enter 翻译</span>
          <span class="panel-meta">{{ store.inputLineCount }} 行</span>
        </div>
        <p class="panel-desc">粘贴 {{ store.sourceLabel }} 建表语句（CREATE TABLE、COMMENT、CREATE INDEX、分区等）</p>

        <div class="panel-editor-shell">
          <SqlEditor
            :model-value="store.inputDdl"
      ...
      <article class="panel">
        <div class="panel-head">
          <div class="panel-head-left">
            <span class="panel-dot" :class="store.targetDb"></span>
            <h2 class="panel-title">{{ store.targetLabel }} 输出</h2>
          </div>
          <button
            class="panel-copy-btn"
            @click="handleCopy"
            :disabled="!store.outputDdl"
            title="复制输出"
          >
    ...
    <div class="status-bar">
      <span class="status-text" :class="{ error: store.ddlStatusText?.includes('失败') }">
        {{ store.ddlStatusText || '就绪' }}
      </span>
    </div>
  </div>
</template>
