...
const features: FeatureGuide[] = [
  ...
  },
  {
    ...
    icon: 'document-text',
    title: '元数据管理',
    summary: '业务字段元数据维护与多版本修订追踪',
    steps: [
      { title: '新增元数据记录', desc: '点击"新增"按钮在表格中添加一行，依次填写中文名称、字段名称、属性类型等关键信息；必要字段会自动校验。' },
      { title: '编辑与业务描述', desc: '直接在单元格中修改字段，点击"业务描述"可展开长文本编辑区，支持搜索、按类型过滤和列排序。' },
      { title: '保存并生成修订', desc: '点击"保存"弹出修订信息窗口，填写版本号（如 v1.0.0）和修订说明，保存后形成一条不可丢失的修订记录。' },
      { title: '查看版本历史', desc: '右侧面板按记录展示完整修订时间线，连接线高亮当前选中记录，可编辑或回看历史修订内容。' },
      { title: '导出与同步', desc: '支持一键导出 CSV / JSON；登录后自动与云端双向同步，状态条提示"已同步 / 同步中 / 同步失败"。' }
    ],
    linkText: '前往使用'
  }
]
