...
async function handleCopy(): Promise<void> {
  ...
  await copyToClipboard(store.funcOutput)
}

function handleInputUpdate(value: string): void {
  store.funcInput = value
}
</script>
  ...
  <div class="func-page">
    <section class="workspace">
      ...
      <article class="panel">
        <div class="panel-head">
          <div class="panel-head-left">
            <span class="panel-dot" :class="store.funcSourceDb"></span>
            <h2 class="panel-title">{{ store.funcSourceLabel }} 输入</h2>
          </div>
          <span class="panel-paste-hint">Ctrl / ⌘ + Enter 翻译</span>
        </div>
        <p class="panel-desc">粘贴 {{ store.funcSourceLabel }} 函数定义（CREATE FUNCTION）</p>

        <div class="panel-editor-shell">
          <SqlEditor
            :model-value="store.funcInput"
      ...
      <article class="panel">
        <div class="panel-head">
          <div class="panel-head-left">
            <span class="panel-dot" :class="store.funcTargetDb"></span>
            <h2 class="panel-title">{{ store.funcTargetLabel }} 输出</h2>
          </div>
          <button
            class="panel-copy-btn"
            @click="handleCopy"
            :disabled="!store.funcOutput"
            title="复制输出"
          >
