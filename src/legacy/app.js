...
function makeTable() {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.createDdlTableModel === 'function') {
    return window.SQLDEV_DDL_UTILS.createDdlTableModel();
  }
...
function makeColumn() {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.createDdlColumnModel === 'function') {
    return window.SQLDEV_DDL_UTILS.createDdlColumnModel();
  }
...
function makeView() {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.createDdlViewModel === 'function') {
    return window.SQLDEV_DDL_UTILS.createDdlViewModel();
  }
...
function splitStatements(sql) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_SQL_UTILS &&
      typeof window.SQLDEV_SQL_UTILS.splitStatements === 'function') {
    return window.SQLDEV_SQL_UTILS.splitStatements(String(sql || ''));
  }
...
function splitColumnDefs(body) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.splitColumnDefinitions === 'function') {
    return window.SQLDEV_DDL_UTILS.splitColumnDefinitions(body);
  }
...
function extractParenBody(str) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.extractParenthesizedBody === 'function') {
    return window.SQLDEV_DDL_UTILS.extractParenthesizedBody(str);
  }
...
function splitListValues(str) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.splitListValues === 'function') {
    return window.SQLDEV_DDL_UTILS.splitListValues(str);
  }
...
function pad(s, n) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.padText === 'function') {
    return window.SQLDEV_DDL_UTILS.padText(s, n);
  }
...
function parseOracleDDL(sql) {
  ...
  for (const stmt of stmts) {
    ...
    if (typeof window !== 'undefined' &&
        window.SQLDEV_DDL_UTILS &&
        typeof window.SQLDEV_DDL_UTILS.extractCreateTableSections === 'function') {
      sections = window.SQLDEV_DDL_UTILS.extractCreateTableSections(s, ctHeader[0].length);
    }
    if (!sections) {
      ...
      sections = {
        body: s.slice(openIdx + 1, bodyEnd),
        trailing: s.slice(bodyEnd + 1).trim()
      };
    }
    let body = sections.body;
    const afterParen = sections.trailing;
    ...
    for (const part of parts) {
      ...
      if (typeof window !== 'undefined' &&
          window.SQLDEV_DDL_UTILS &&
          typeof window.SQLDEV_DDL_UTILS.parseOracleTableConstraintDefinition === 'function') {
        const parsed = window.SQLDEV_DDL_UTILS.parseOracleTableConstraintDefinition(p, tbl.name);
        if (parsed) {
          if (parsed.primaryKey) { tbl.primaryKey = parsed.primaryKey; continue; }
          if (parsed.uniqueKey) { tbl.uniqueKeys.push(parsed.uniqueKey); continue; }
          if (parsed.foreignKey) { tbl.foreignKeys.push(parsed.foreignKey); continue; }
  ...
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.applyOracleCommentStatements === 'function') {
    window.SQLDEV_DDL_UTILS.applyOracleCommentStatements(tables, stmts);
    window.SQLDEV_DDL_UTILS.applyOracleIndexStatements(tables, stmts);
    window.SQLDEV_DDL_UTILS.applyOraclePrimaryKeyStatements(tables, stmts);
    window.SQLDEV_DDL_UTILS.applyOracleForeignKeyStatements(tables, stmts);
  } else {
    // Phase 2: COMMENT ON
    for (const stmt of stmts) {
      const s = stmt.trim();
...
function parseOracleColumn(def) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.parseOracleColumnDefinition === 'function') {
    return window.SQLDEV_DDL_UTILS.parseOracleColumnDefinition(def);
  }
...
function parseMySQLDDL(sql) {
  ...
  for (const stmt of stmts) {
    ...
    for (const part of parts) {
      ...
      if (typeof window !== 'undefined' &&
          window.SQLDEV_DDL_UTILS &&
          typeof window.SQLDEV_DDL_UTILS.parseMySqlTableConstraintDefinition === 'function') {
        const parsed = window.SQLDEV_DDL_UTILS.parseMySqlTableConstraintDefinition(p, tbl.name);
        if (parsed) {
          if (parsed.primaryKey) { tbl.primaryKey = parsed.primaryKey; continue; }
          if (parsed.uniqueKey) { tbl.uniqueKeys.push(parsed.uniqueKey); continue; }
          if (parsed.index) { tbl.indexes.push(parsed.index); continue; }
          if (parsed.foreignKey) { tbl.foreignKeys.push(parsed.foreignKey); continue; }
          if (parsed.ignore) continue;
        }
      }
...
function parseMySQLColumn(def) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.parseMySqlColumnDefinition === 'function') {
    return window.SQLDEV_DDL_UTILS.parseMySqlColumnDefinition(def);
  }
  const m = def.match(/^[`"']?(\w+)[`"']?\s+(.+)$/i);
  if (!m) return null;
  const col = makeColumn();
...
function parsePostgreSQLDDL(sql) {
  ...
  for (const stmt of stmts) {
    ...
    if (/^CREATE\s+TABLE\s+\w+\s+PARTITION\s+OF\b/i.test(s)) {
      if (!(typeof window !== 'undefined' &&
            window.SQLDEV_DDL_UTILS &&
            typeof window.SQLDEV_DDL_UTILS.applyPostgresPartitionStatements === 'function')) {
    ...
    if (!sections) {
      ...
      sections = {
        body: s.slice(openIdx + 1, _bodyEnd),
        trailing: s.slice(_bodyEnd + 1).trim()
      };
    }
    const pgBody = sections.body;
    const afterParen = sections.trailing;
    ...
    for (const part of parts) {
      ...
      if (typeof window !== 'undefined' &&
          window.SQLDEV_DDL_UTILS &&
          typeof window.SQLDEV_DDL_UTILS.parsePostgresTableConstraintDefinition === 'function') {
        const parsed = window.SQLDEV_DDL_UTILS.parsePostgresTableConstraintDefinition(p, tbl.name);
        if (parsed) {
          if (parsed.primaryKey) { tbl.primaryKey = parsed.primaryKey; continue; }
          if (parsed.uniqueKey) { tbl.uniqueKeys.push(parsed.uniqueKey); continue; }
          if (parsed.foreignKey) { tbl.foreignKeys.push(parsed.foreignKey); continue; }
  ...
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.applyPostgresCommentStatements === 'function') {
    window.SQLDEV_DDL_UTILS.applyPostgresCommentStatements(tables, stmts);
    window.SQLDEV_DDL_UTILS.applyPostgresIndexStatements(tables, stmts);
    window.SQLDEV_DDL_UTILS.applyPostgresForeignKeyStatements(tables, stmts);
    window.SQLDEV_DDL_UTILS.applyPostgresPartitionStatements(tables, stmts, splitListValues);
  } else {
    // COMMENT ON
    for (const stmt of stmts) {
      const s = stmt.trim();
...
function parsePGColumn(def) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.parsePostgresColumnDefinition === 'function') {
    return window.SQLDEV_DDL_UTILS.parsePostgresColumnDefinition(def);
  }
  const m = def.match(/^["']?(\w+)["']?\s+(.+)$/i);
  if (!m) return null;
  const col = makeColumn();
...
function _parseDdlSource(src) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.parseDdlRuleSource === 'function') {
    return window.SQLDEV_DDL_UTILS.parseDdlRuleSource(src);
  }
  var s = src.trim();
  var cond = null;
...
function _matchDdlSource(col, parsed) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.matchesDdlRuleSource === 'function') {
    return window.SQLDEV_DDL_UTILS.matchesDdlRuleSource(col, parsed);
  }
...
function _applyDdlTarget(col, targetStr) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.applyDdlRuleTarget === 'function') {
    return window.SQLDEV_DDL_UTILS.applyDdlRuleTarget(col, targetStr);
  }
...
function _mapTypeByRules(col, rules) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.mapDdlTypeByRules === 'function') {
    return window.SQLDEV_DDL_UTILS.mapDdlTypeByRules(col, rules);
  }
...
function convertDefault(val, from, to) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.convertDdlDefaultValue === 'function') {
    return window.SQLDEV_DDL_UTILS.convertDdlDefaultValue(val, from, to);
  }
  if (!val) return null;
...
function generateOracleDDL(tables, fromDb) {
  ...
  for (const tbl of tables) {
    ...
    lines.push('');
    var __oracleBuiltByBridge = false;
    if (typeof window !== 'undefined' &&
        window.SQLDEV_DDL_UTILS &&
        typeof window.SQLDEV_DDL_UTILS.buildOracleCommentLines === 'function' &&
        typeof window.SQLDEV_DDL_UTILS.buildOracleIndexLines === 'function' &&
        typeof window.SQLDEV_DDL_UTILS.buildOracleForeignKeyLines === 'function') {
      var oracleCommentLines = window.SQLDEV_DDL_UTILS.buildOracleCommentLines(tname, tbl.comment, tbl.columns);
      var oracleIndexLines = window.SQLDEV_DDL_UTILS.buildOracleIndexLines(tname, tbl.uniqueKeys, tbl.indexes);
      var oracleForeignKeyLines = window.SQLDEV_DDL_UTILS.buildOracleForeignKeyLines(tname, tbl.foreignKeys);
      if (oracleCommentLines.length) { lines.push(...oracleCommentLines); lines.push(''); }
      if (oracleIndexLines.length) { lines.push(...oracleIndexLines); lines.push(''); }
      if (oracleForeignKeyLines.length) { lines.push(...oracleForeignKeyLines); lines.push(''); }
      __oracleBuiltByBridge = true;
    }
    if (!__oracleBuiltByBridge) if (tbl.comment) lines.push("COMMENT ON TABLE " + tname + " IS '" + tbl.comment.replace(/'/g, "''") + "';");
    if (!__oracleBuiltByBridge) for (const col of tbl.columns) { if (col.comment) lines.push("COMMENT ON COLUMN " + tname + "." + col.name.toUpperCase() + " IS '" + col.comment.replace(/'/g, "''") + "';"); }
    if (!__oracleBuiltByBridge && (tbl.columns.some(c=>c.comment) || tbl.comment)) lines.push('');
    if (!__oracleBuiltByBridge) for (const uk of tbl.uniqueKeys) lines.push('CREATE UNIQUE INDEX ' + uk.name.toUpperCase() + ' ON ' + tname + '(' + uk.columns.map(c=>c.toUpperCase()).join(', ') + ');');
    if (!__oracleBuiltByBridge) for (const idx of tbl.indexes) lines.push('CREATE INDEX ' + idx.name.toUpperCase() + ' ON ' + tname + '(' + idx.columns.map(c=>c.toUpperCase()).join(', ') + ');');
    if (!__oracleBuiltByBridge && (tbl.uniqueKeys.length || tbl.indexes.length)) lines.push('');
    if (!__oracleBuiltByBridge) for (const fk of tbl.foreignKeys) {
    ...
    if (!__oracleBuiltByBridge && tbl.foreignKeys.length) lines.push('');
  }
  return lines.join('\n').trim();
}
...
function generateMySQLDDL(tables, fromDb) {
  ...
  for (const tbl of tables) {
    ...
    var __mySqlConstraintsBuiltByBridge = false;
    if (typeof window !== 'undefined' &&
        window.SQLDEV_DDL_UTILS &&
        typeof window.SQLDEV_DDL_UTILS.buildMySqlInlineConstraintLines === 'function') {
      colLines.push.apply(colLines, window.SQLDEV_DDL_UTILS.buildMySqlInlineConstraintLines(tbl));
      __mySqlConstraintsBuiltByBridge = true;
    }
    if (!__mySqlConstraintsBuiltByBridge && tbl.primaryKey) {
    ...
    if (!__mySqlConstraintsBuiltByBridge) for (const uk of tbl.uniqueKeys) colLines.push('    UNIQUE KEY `' + uk.name.toLowerCase() + '` (' + uk.columns.map(c=>'`'+c.toLowerCase()+'`').join(', ') + ')');
    if (!__mySqlConstraintsBuiltByBridge) for (const idx of tbl.indexes) colLines.push('    KEY `' + idx.name.toLowerCase() + '` (' + idx.columns.map(c=>'`'+c.toLowerCase()+'`').join(', ') + ')');
    if (!__mySqlConstraintsBuiltByBridge) for (const fk of tbl.foreignKeys) {
...
function generatePostgreSQLDDL(tables, fromDb) {
  ...
  for (const tbl of tables) {
    ...
    var __pgBuiltByBridge = false;
    if (typeof window !== 'undefined' &&
        window.SQLDEV_DDL_UTILS &&
        typeof window.SQLDEV_DDL_UTILS.buildPostgresCommentLines === 'function' &&
        typeof window.SQLDEV_DDL_UTILS.buildPostgresIndexLines === 'function' &&
        typeof window.SQLDEV_DDL_UTILS.buildPostgresForeignKeyLines === 'function') {
      var pgCommentLines = window.SQLDEV_DDL_UTILS.buildPostgresCommentLines(tname, tbl.comment, tbl.columns);
      var pgIndexLines = window.SQLDEV_DDL_UTILS.buildPostgresIndexLines(tname, tbl.uniqueKeys, tbl.indexes);
      var pgForeignKeyLines = window.SQLDEV_DDL_UTILS.buildPostgresForeignKeyLines(tname, tbl.foreignKeys);
      if (pgCommentLines.length) { lines.push(...pgCommentLines); lines.push(''); }
      if (pgIndexLines.length) { lines.push(...pgIndexLines); lines.push(''); }
      if (pgForeignKeyLines.length) { lines.push(...pgForeignKeyLines); lines.push(''); }
      __pgBuiltByBridge = true;
    }
    if (!__pgBuiltByBridge) if (tbl.comment) lines.push("COMMENT ON TABLE " + tname + " IS '" + tbl.comment.replace(/'/g, "''") + "';");
    if (!__pgBuiltByBridge) for (const col of tbl.columns) { if (col.comment) lines.push("COMMENT ON COLUMN " + tname + "." + col.name.toLowerCase() + " IS '" + col.comment.replace(/'/g, "''") + "';"); }
    if (!__pgBuiltByBridge && (tbl.columns.some(c=>c.comment) || tbl.comment)) lines.push('');
    if (!__pgBuiltByBridge) for (const uk of tbl.uniqueKeys) lines.push('CREATE UNIQUE INDEX ' + uk.name.toLowerCase() + ' ON ' + tname + '(' + uk.columns.map(c=>c.toLowerCase()).join(', ') + ');');
    if (!__pgBuiltByBridge) for (const idx of tbl.indexes) lines.push('CREATE INDEX ' + idx.name.toLowerCase() + ' ON ' + tname + '(' + idx.columns.map(c=>c.toLowerCase()).join(', ') + ');');
    if (!__pgBuiltByBridge && (tbl.uniqueKeys.length || tbl.indexes.length)) lines.push('');
    if (!__pgBuiltByBridge) for (const fk of tbl.foreignKeys) {
    ...
    if (!__pgBuiltByBridge && tbl.foreignKeys.length) lines.push('');
  }
  return lines.join('\n').trim();
}
...
function parseViews(sql, sourceDb) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.parseViewStatements === 'function') {
    return window.SQLDEV_DDL_UTILS.parseViewStatements(sql, splitStatements, makeView);
  }
  var views = [];
...
function transformViewQuery(query, fromDb, toDb) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.transformViewQueryText === 'function') {
    return window.SQLDEV_DDL_UTILS.transformViewQueryText(query, fromDb, toDb, transformBody);
  }
...
function generateOracleViews(views, fromDb) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.generateOracleViewStatements === 'function') {
    return window.SQLDEV_DDL_UTILS.generateOracleViewStatements(views, fromDb, transformViewQuery);
  }
  var lines = [];
...
function generateMySQLViews(views, fromDb) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.generateMySqlViewStatements === 'function') {
    return window.SQLDEV_DDL_UTILS.generateMySqlViewStatements(views, fromDb, transformViewQuery);
  }
  var lines = [];
  for (var i = 0; i < views.length; i++) {
    var vw = views[i];
...
function generatePGViews(views, fromDb) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.generatePostgresViewStatements === 'function') {
    return window.SQLDEV_DDL_UTILS.generatePostgresViewStatements(views, fromDb, transformViewQuery);
  }
  var lines = [];
  for (var i = 0; i < views.length; i++) {
    var vw = views[i];
...
function parseExtraDDL(sql, sourceDb) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.parseExtraDdlStatements === 'function') {
    return window.SQLDEV_DDL_UTILS.parseExtraDdlStatements(sql, sourceDb, splitStatements);
  }
...
function parseAlterColType(table, column, typeStr, action) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.parseAlterColumnTypeDefinition === 'function') {
    return window.SQLDEV_DDL_UTILS.parseAlterColumnTypeDefinition(table, column, typeStr, action);
  }
...
function parseAddCol(table, column, typeStr, rest) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.parseAddColumnDefinition === 'function') {
    return window.SQLDEV_DDL_UTILS.parseAddColumnDefinition(table, column, typeStr, rest);
  }
...
function convertExtraColType(typeStr, length, precision, scale, sourceDb, targetDb) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.convertExtraColumnType === 'function') {
    return window.SQLDEV_DDL_UTILS.convertExtraColumnType(
      typeStr,
      length,
      precision,
      scale,
      sourceDb,
      targetDb,
      function(nextType, nextLength, nextPrecision, nextScale, fromDbName, toDbName) {
        const col = { type: nextType, length: nextLength, precision: nextPrecision, scale: nextScale };
        if (fromDbName === 'oracle' && toDbName === 'mysql') return oracleTypeToMySQL(col);
        if (fromDbName === 'oracle' && toDbName === 'postgresql') return oracleTypeToPG(col);
        if (fromDbName === 'mysql' && toDbName === 'oracle') return mysqlTypeToOracle(col);
        if (fromDbName === 'mysql' && toDbName === 'postgresql') return mysqlTypeToPG(col);
        if (fromDbName === 'postgresql' && toDbName === 'oracle') return pgTypeToOracle(col);
        if (fromDbName === 'postgresql' && toDbName === 'mysql') return pgTypeToMySQL(col);
        if (nextLength) return nextType + '(' + nextLength + ')';
        if (nextPrecision && nextScale) return nextType + '(' + nextPrecision + ',' + nextScale + ')';
        if (nextPrecision) return nextType + '(' + nextPrecision + ')';
        return nextType;
      }
    );
  }
...
function generateExtraDDL(parsed, sourceDb, targetDb) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.generateExtraDdlStatements === 'function') {
    return window.SQLDEV_DDL_UTILS.generateExtraDdlStatements(
      parsed,
      sourceDb,
      targetDb,
      function(typeStr, length, precision, scale, fromDbName, toDbName) {
        return convertExtraColType(typeStr, length, precision, scale, fromDbName, toDbName);
      }
    );
  }
  const lines = [];
...
function convertDDL(input, sourceDb, targetDb) {
  if (typeof window !== 'undefined' &&
      window.SQLDEV_DDL_UTILS &&
      typeof window.SQLDEV_DDL_UTILS.convertDdlOrchestrated === 'function') {
    return window.SQLDEV_DDL_UTILS.convertDdlOrchestrated(input, sourceDb, targetDb, {
      labels: DB_LABELS,
      parseOracleDDL: parseOracleDDL,
      parseMySqlDDL: parseMySQLDDL,
      parsePostgresDDL: parsePostgreSQLDDL,
      parseViews: parseViews,
      parseExtraDDL: parseExtraDDL,
      generateOracleDDL: generateOracleDDL,
      generateMySqlDDL: generateMySQLDDL,
      generatePostgresDDL: generatePostgreSQLDDL,
      generateOracleViews: generateOracleViews,
      generateMySqlViews: generateMySQLViews,
      generatePostgresViews: generatePGViews,
      generateExtraDDL: generateExtraDDL
    });
  }
...
function _saveDdlRulesToStorage() {
  if (window.SQLDEV_RULE_STORAGE_UTILS && typeof window.SQLDEV_RULE_STORAGE_UTILS.saveDdlRulesToStorage === 'function') {
    var ddlPersist = window.SQLDEV_RULE_STORAGE_UTILS.saveDdlRulesToStorage(localStorage, _STORAGE_KEY_DDL, _ddlRulesData);
    _persistError = ddlPersist.error || '';
    return !!ddlPersist.ok;
  }
...
function _saveBodyRulesToStorage() {
  if (window.SQLDEV_RULE_STORAGE_UTILS && typeof window.SQLDEV_RULE_STORAGE_UTILS.saveBodyRulesToStorage === 'function') {
    var bodyPersist = window.SQLDEV_RULE_STORAGE_UTILS.saveBodyRulesToStorage(localStorage, _STORAGE_KEY_BODY, _bodyRulesData);
    _persistError = bodyPersist.error || '';
    return !!bodyPersist.ok;
  }
  var out = {};
...
function _persistRules() {
  if (window.SQLDEV_RULE_STORAGE_UTILS && typeof window.SQLDEV_RULE_STORAGE_UTILS.persistRulesToStorage === 'function') {
    var result = window.SQLDEV_RULE_STORAGE_UTILS.persistRulesToStorage(
      localStorage,
      _STORAGE_KEY_DDL,
      _STORAGE_KEY_BODY,
      _ddlRulesData,
...
function _loadDdlRulesFromStorage() {
  if (window.SQLDEV_RULE_STORAGE_UTILS && typeof window.SQLDEV_RULE_STORAGE_UTILS.loadDdlRulesFromStorage === 'function') {
    return window.SQLDEV_RULE_STORAGE_UTILS.loadDdlRulesFromStorage(localStorage, _STORAGE_KEY_DDL);
  }
  try {
...
function _loadBodyRulesFromStorage() {
  if (window.SQLDEV_RULE_STORAGE_UTILS && typeof window.SQLDEV_RULE_STORAGE_UTILS.loadBodyRulesFromStorage === 'function') {
    return window.SQLDEV_RULE_STORAGE_UTILS.loadBodyRulesFromStorage(localStorage, _STORAGE_KEY_BODY, _bodyRulesDefault);
  }
  try {
...
(function _hydrateRules() {
  ...
  if (ddl) {
    if (window.SQLDEV_RULE_STORAGE_UTILS && typeof window.SQLDEV_RULE_STORAGE_UTILS.hydrateRulesData === 'function') {
      window.SQLDEV_RULE_STORAGE_UTILS.hydrateRulesData(_ddlRulesData, ddl);
    } else {
      for (var k in ddl) { if (_ddlRulesData.hasOwnProperty(k)) _ddlRulesData[k] = ddl[k]; }
    }
  }
  var body = _loadBodyRulesFromStorage();
  if (body) {
    if (window.SQLDEV_RULE_STORAGE_UTILS && typeof window.SQLDEV_RULE_STORAGE_UTILS.hydrateRulesData === 'function') {
      window.SQLDEV_RULE_STORAGE_UTILS.hydrateRulesData(_bodyRulesData, body);
    } else {
      for (var k in body) { if (_bodyRulesData.hasOwnProperty(k)) _bodyRulesData[k] = body[k]; }
    }
  }
})();
...
function mapParamType(typeStr, fromDb, toDb) {
  if (window.SQLDEV_RULE_STORAGE_UTILS && typeof window.SQLDEV_RULE_STORAGE_UTILS.mapParamTypeByRules === 'function') {
    return window.SQLDEV_RULE_STORAGE_UTILS.mapParamTypeByRules(typeStr, fromDb, toDb, _bodyRulesData);
  }
...
function _getTypeCategories(fromDb, toDb) {
  if (window.SQLDEV_RULE_STORAGE_UTILS && typeof window.SQLDEV_RULE_STORAGE_UTILS.getBodyRuleCategories === 'function') {
    return window.SQLDEV_RULE_STORAGE_UTILS.getBodyRuleCategories(fromDb, toDb);
  }
...
function transformBody(body, fromDb, toDb) {
  if (window.SQLDEV_RULE_STORAGE_UTILS && typeof window.SQLDEV_RULE_STORAGE_UTILS.transformBodyByRules === 'function') {
    return window.SQLDEV_RULE_STORAGE_UTILS.transformBodyByRules(body, fromDb, toDb, _bodyRulesData);
  }
...
function _parseOracleParam(paramStr) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.parseOracleRoutineParam === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.parseOracleRoutineParam(paramStr);
  }
...
function _parseMySQLParam(paramStr) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.parseMySqlRoutineParam === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.parseMySqlRoutineParam(paramStr);
  }
  const s = paramStr.trim();
  if (!s) return null;
...
function _parsePGParam(paramStr) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.parsePostgresRoutineParam === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.parsePostgresRoutineParam(paramStr);
  }
  const s = paramStr.trim();
  if (!s) return null;
...
function _parseOracleVarDecl(declBlock) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.parseOracleVariableDeclarations === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.parseOracleVariableDeclarations(declBlock);
  }
  const vars = [];
...
function _parseOracleFunction(input) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.parseOracleFunctionDefinition === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.parseOracleFunctionDefinition(input);
  }
  ...
  const params = (
    window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.splitRoutineParamList === 'function'
      ? window.SQLDEV_ROUTINE_UTILS.splitRoutineParamList(paramStr)
      : _splitParamList(paramStr)
  ).map(_parseOracleParam).filter(Boolean);
...
function _parseMySQLFunction(input) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.parseMySqlFunctionDefinition === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.parseMySqlFunctionDefinition(input);
  }
  ...
  const extracted = window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.extractMySqlRoutineDeclarations === 'function'
    ? window.SQLDEV_ROUTINE_UTILS.extractMySqlRoutineDeclarations(bodyContent)
    : null;
  const vars = extracted ? extracted.vars : [];
  let body = extracted ? extracted.body : '';
  if (!extracted) {
    ...
    body = nonDeclLines.join('\n');
  }
  const params = (
    window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.splitRoutineParamList === 'function'
      ? window.SQLDEV_ROUTINE_UTILS.splitRoutineParamList(paramStr)
      : _splitParamList(paramStr)
  ).map(_parseMySQLParam).filter(Boolean);
  return { name: name, params: params, returnType: returnType, vars: vars, body: body };
}

function _parsePGFunction(input) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.parsePostgresFunctionDefinition === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.parsePostgresFunctionDefinition(input);
  }
  let src = input.trim();
  ...
  const extracted = window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.extractPostgresRoutineDeclarations === 'function'
    ? window.SQLDEV_ROUTINE_UTILS.extractPostgresRoutineDeclarations(inner)
    : null;
  const vars = extracted ? extracted.vars : [];
  const body = extracted ? extracted.body : (function() {
    const localVars = [];
    ...
    if (declareMatch) {
      ...
      for (let i = 0; i < declParts.length; i++) {
        ...
        if (cursorMatch) {
          localVars.push({ name: cursorMatch[1], cursor: true, query: cursorMatch[2].trim(), type: 'CURSOR' });
          continue;
        }
        const vm = s.match(/^(\w+)\s+(.+?)(?:\s*:=\s*(.+))?$/i);
        if (vm) localVars.push({ name: vm[1], type: vm[2].trim(), defaultVal: vm[3] ? vm[3].trim() : null });
      }
    }
    while (localVars.length) vars.push(localVars.shift());
  ...
  const params = (
    window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.splitRoutineParamList === 'function'
      ? window.SQLDEV_ROUTINE_UTILS.splitRoutineParamList(paramStr)
      : _splitParamList(paramStr)
  ).map(_parsePGParam).filter(Boolean);
  return { name: name, params: params, returnType: returnType, vars: vars, body: body };
}
...
function _genOracleFunction(name, params, returnType, vars, body) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.generateOracleFunctionStatement === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.generateOracleFunctionStatement(name, params, returnType, vars, body);
  }
...
function _genMySQLFunction(name, params, returnType, vars, body) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.generateMySqlFunctionStatement === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.generateMySqlFunctionStatement(name, params, returnType, vars, body);
  }
...
function _genPGFunction(name, params, returnType, vars, body) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.generatePostgresFunctionStatement === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.generatePostgresFunctionStatement(name, params, returnType, vars, body);
  }
  let out = 'CREATE OR REPLACE FUNCTION ' + name + '(\n';
  const pLines = params.map(function(p) {
    let line = '  ' + p.name;
...
function _parseOracleProcedure(input) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.parseOracleProcedureDefinition === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.parseOracleProcedureDefinition(input);
  }
  let src = input.replace(/\s*\/\s*$/, '').trim();
  src = src.replace(/(\bEND\b\s*\w*\s*;?)\s*(?:\/\s*)?\s*(?:\n\s*--[^\n]*)*\s*$/i, '$1');
...
function _parseMySQLProcedure(input) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.parseMySqlProcedureDefinition === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.parseMySqlProcedureDefinition(input);
  }
  let src = input.replace(/^\s*DELIMITER\s+\S+\s*$/gim, '').trim();
  src = src.replace(/\$\$/g, '').trim();
  src = src.replace(/(\bEND\b\s*\w*\s*;?)\s*(?:\n\s*--[^\n]*)*\s*$/i, '$1');
...
function _parsePGProcedure(input) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.parsePostgresProcedureDefinition === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.parsePostgresProcedureDefinition(input);
  }
  let src = input.trim();
  src = src.replace(/(\$\$\s*;?)\s*(?:\n\s*--[^\n]*)*\s*$/i, '$1');
...
function _genOracleProcedure(name, params, vars, body) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.generateOracleProcedureStatement === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.generateOracleProcedureStatement(name, params, vars, body);
  }
...
function _genMySQLProcedure(name, params, vars, body) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.generateMySqlProcedureStatement === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.generateMySqlProcedureStatement(name, params, vars, body);
  }
...
function _genPGProcedure(name, params, vars, body) {
  if (window.SQLDEV_ROUTINE_UTILS && typeof window.SQLDEV_ROUTINE_UTILS.generatePostgresProcedureStatement === 'function') {
    return window.SQLDEV_ROUTINE_UTILS.generatePostgresProcedureStatement(name, params, vars, body);
  }
...
const app = createApp({
  setup() {
    ...
    const sidebarCollapsed = ref(
      window.SQLDEV_PREFERENCE_UTILS && typeof window.SQLDEV_PREFERENCE_UTILS.getSidebarCollapsedPreference === 'function'
        ? window.SQLDEV_PREFERENCE_UTILS.getSidebarCollapsedPreference(localStorage)
        : false
    );
    ...
    function toggleSidebar() {
      ...
      if (window.SQLDEV_PREFERENCE_UTILS && typeof window.SQLDEV_PREFERENCE_UTILS.saveSidebarCollapsedPreference === 'function') {
        window.SQLDEV_PREFERENCE_UTILS.saveSidebarCollapsedPreference(localStorage, sidebarCollapsed.value);
      } else {
    ...
    const themeMode = ref(
      window.SQLDEV_PREFERENCE_UTILS && typeof window.SQLDEV_PREFERENCE_UTILS.getThemePreference === 'function'
        ? window.SQLDEV_PREFERENCE_UTILS.getThemePreference(localStorage)
    ...
    function toggleTheme() {
      ...
      if (window.SQLDEV_PREFERENCE_UTILS && typeof window.SQLDEV_PREFERENCE_UTILS.saveThemePreference === 'function') {
        window.SQLDEV_PREFERENCE_UTILS.saveThemePreference(localStorage, themeMode.value);
      } else {
    ...
    */
    ...
    function _isLeapYear(year) {
      if (typeof window !== 'undefined' && window.SQLDEV_ID_TOOL_UTILS) {
        return window.SQLDEV_ID_TOOL_UTILS.isLeapYear(year);
      }
    ...
    function _isValidDateParts(year, month, day) {
      if (typeof window !== 'undefined' && window.SQLDEV_ID_TOOL_UTILS) {
        return window.SQLDEV_ID_TOOL_UTILS.isValidDateParts(year, month, day);
      }
    ...
    function _parseYmdFromIsoDate(value) {
      if (typeof window !== 'undefined' && window.SQLDEV_ID_TOOL_UTILS) {
        return window.SQLDEV_ID_TOOL_UTILS.parseYmdFromIsoDate(value);
      }
    ...
    function _validateBirthYmd8(ymd8) {
      if (typeof window !== 'undefined' && window.SQLDEV_ID_TOOL_UTILS) {
        return window.SQLDEV_ID_TOOL_UTILS.validateBirthYmd8(ymd8, { maxDate: idBirthMax.value });
      }
    ...
    function _calcIdCheckDigit(id17) {
      if (typeof window !== 'undefined' && window.SQLDEV_ID_TOOL_UTILS) {
        return window.SQLDEV_ID_TOOL_UTILS.calcIdCardCheckDigit(id17);
      }
    ...
    function _randomSeqByGender(gender) {
      if (typeof window !== 'undefined' && window.SQLDEV_ID_TOOL_UTILS) {
        return window.SQLDEV_ID_TOOL_UTILS.randomSequenceByGender(gender, _randomInt);
      }
    ...
    function _zwLoadHistory() {
      if (window.SQLDEV_ZIWEI_HISTORY_UTILS && typeof window.SQLDEV_ZIWEI_HISTORY_UTILS.loadZiweiHistoryRecords === 'function') {
        return window.SQLDEV_ZIWEI_HISTORY_UTILS.loadZiweiHistoryRecords(localStorage, ZW_HISTORY_KEY, 30);
      }
      try {
    ...
    function _zwSaveHistory(historyList) {
      if (window.SQLDEV_ZIWEI_HISTORY_UTILS && typeof window.SQLDEV_ZIWEI_HISTORY_UTILS.saveZiweiHistoryRecords === 'function') {
        window.SQLDEV_ZIWEI_HISTORY_UTILS.saveZiweiHistoryRecords(localStorage, ZW_HISTORY_KEY, historyList);
        return;
      }
      try {
    ...
    function _zwBuildHistoryLabel() {
      if (window.SQLDEV_ZIWEI_HISTORY_UTILS && typeof window.SQLDEV_ZIWEI_HISTORY_UTILS.buildZiweiHistoryLabel === 'function') {
        return window.SQLDEV_ZIWEI_HISTORY_UTILS.buildZiweiHistoryLabel({
          profileName: ziweiProfileName.value,
          gender: ziweiGender.value,
          solarYear: ziweiSolarYear.value,
          solarMonth: ziweiSolarMonth.value,
          solarDay: ziweiSolarDay.value
        });
      }
    ...
    function _zwPushHistory(chart) {
      if (!chart || !chart.center) return;
      var record = window.SQLDEV_ZIWEI_HISTORY_UTILS && typeof window.SQLDEV_ZIWEI_HISTORY_UTILS.createZiweiHistoryRecord === 'function'
        ? window.SQLDEV_ZIWEI_HISTORY_UTILS.createZiweiHistoryRecord({
          profileName: ziweiProfileName.value,
          calendarType: ziweiCalendarType.value,
          solarYear: ziweiSolarYear.value,
          solarMonth: ziweiSolarMonth.value,
          solarDay: ziweiSolarDay.value,
          lunarYear: ziweiLunarYear.value,
          lunarMonth: ziweiLunarMonth.value,
          lunarDay: ziweiLunarDay.value,
          lunarLeap: ziweiLunarLeap.value,
          birthHour: ziweiBirthHour.value,
          birthMinute: ziweiBirthMinute.value,
          gender: ziweiGender.value,
          clockMode: ziweiClockMode.value,
          timezoneOffset: ziweiTimezoneOffset.value,
          longitude: ziweiLongitude.value,
          xiaoXianRule: ziweiXiaoXianRule.value,
          liuNianRule: ziweiLiuNianRule.value,
          school: ziweiSchool.value
        }, chart)
        : {
    ...
    function formatZiweiHistoryTime(ts) {
      if (window.SQLDEV_ZIWEI_PRESENTATION_UTILS && typeof window.SQLDEV_ZIWEI_PRESENTATION_UTILS.formatZiweiHistoryTime === 'function') {
        return window.SQLDEV_ZIWEI_PRESENTATION_UTILS.formatZiweiHistoryTime(ts);
      }
      var ms = Number(ts);
    ...
    function formatZiweiDurationText(ms) {
      if (window.SQLDEV_ZIWEI_PRESENTATION_UTILS && typeof window.SQLDEV_ZIWEI_PRESENTATION_UTILS.formatZiweiDurationText === 'function') {
        return window.SQLDEV_ZIWEI_PRESENTATION_UTILS.formatZiweiDurationText(ms);
      }
    ...
    function buildZiweiShareLink() {
      if (window.SQLDEV_ZIWEI_SHARE_UTILS && typeof window.SQLDEV_ZIWEI_SHARE_UTILS.buildZiweiShareLink === 'function') {
        return window.SQLDEV_ZIWEI_SHARE_UTILS.buildZiweiShareLink(
          typeof window === 'undefined' ? null : window.location
        );
      }
      if (typeof window === 'undefined' || !window.location) return '';
    ...
    function _zwTrimText(value, maxLen) {
      if (window.SQLDEV_ZIWEI_AI_UTILS && typeof window.SQLDEV_ZIWEI_AI_UTILS.trimZiweiText === 'function') {
        return window.SQLDEV_ZIWEI_AI_UTILS.trimZiweiText(value, maxLen);
      }
    ...
    function _zwBuildAiPayload(chart) {
      if (window.SQLDEV_ZIWEI_AI_UTILS && typeof window.SQLDEV_ZIWEI_AI_UTILS.buildZiweiAiPayload === 'function') {
        return window.SQLDEV_ZIWEI_AI_UTILS.buildZiweiAiPayload(chart, {
          profileName: ziweiProfileName.value,
          ruleSummary: ziweiAnalysis.value,
    ...
    function _zwLegacyBuildAiPayloadCompact(chart) {
      if (window.SQLDEV_ZIWEI_AI_UTILS && typeof window.SQLDEV_ZIWEI_AI_UTILS.buildZiweiAiPayloadLegacyCompact === 'function') {
        return window.SQLDEV_ZIWEI_AI_UTILS.buildZiweiAiPayloadLegacyCompact(_zwBuildAiPayload(chart));
      }
    ...
    async function _zwParseInvokeError(err) {
      if (window.SQLDEV_ZIWEI_AI_UTILS && typeof window.SQLDEV_ZIWEI_AI_UTILS.parseZiweiInvokeError === 'function') {
        return await window.SQLDEV_ZIWEI_AI_UTILS.parseZiweiInvokeError(err);
      }
      var status = 0;
      var detail = '';
    ...
    function _zwIsComputeResourceErrorMessage(raw) {
      if (window.SQLDEV_ZIWEI_AI_UTILS && typeof window.SQLDEV_ZIWEI_AI_UTILS.isZiweiComputeResourceError === 'function') {
        return window.SQLDEV_ZIWEI_AI_UTILS.isZiweiComputeResourceError(raw);
      }
    ...
    function _zwIsAiRateLimitErrorMessage(raw) {
      if (window.SQLDEV_ZIWEI_AI_UTILS && typeof window.SQLDEV_ZIWEI_AI_UTILS.isZiweiAiRateLimitError === 'function') {
        return window.SQLDEV_ZIWEI_AI_UTILS.isZiweiAiRateLimitError(raw);
      }
      var text = String(raw || '').toLowerCase();
      if (!text) return false;
    ...
    function _zwBuildAiPayloadCompact(chart) {
      if (window.SQLDEV_ZIWEI_AI_UTILS && typeof window.SQLDEV_ZIWEI_AI_UTILS.buildZiweiAiPayloadCompact === 'function') {
        return window.SQLDEV_ZIWEI_AI_UTILS.buildZiweiAiPayloadCompact(_zwBuildAiPayload(chart));
      }
      var payload = _zwBuildAiPayload(chart);
      if (!payload || typeof payload !== 'object') return payload;
    ...
    function _zwBuildAiPayloadLite(chart) {
      if (window.SQLDEV_ZIWEI_AI_UTILS && typeof window.SQLDEV_ZIWEI_AI_UTILS.buildZiweiAiPayloadLite === 'function') {
        return window.SQLDEV_ZIWEI_AI_UTILS.buildZiweiAiPayloadLite(_zwBuildAiPayload(chart));
      }
    ...
    const fileEncoding = ref(
      window.SQLDEV_PREFERENCE_UTILS && typeof window.SQLDEV_PREFERENCE_UTILS.getFileEncodingPreference === 'function'
        ? window.SQLDEV_PREFERENCE_UTILS.getFileEncodingPreference(localStorage)
        : (localStorage.getItem('fileEncoding') || 'UTF-8')
    );
    ...
    watch(fileEncoding, function(value) {
      if (window.SQLDEV_PREFERENCE_UTILS && typeof window.SQLDEV_PREFERENCE_UTILS.saveFileEncodingPreference === 'function') {
        window.SQLDEV_PREFERENCE_UTILS.saveFileEncodingPreference(localStorage, value);
      } else {
    ...
    function clipboardWrite(text) {
      ...
      if (typeof window !== 'undefined' &&
          window.SQLDEV_BROWSER_UTILS &&
          typeof window.SQLDEV_BROWSER_UTILS.copyTextToClipboard === 'function') {
        return window.SQLDEV_BROWSER_UTILS.copyTextToClipboard(content).then(function(ok) {
          statusText.value = ok ? '\u5DF2\u590D\u5236\u5230\u526A\u8D34\u677F' : '\u590D\u5236\u5931\u8D25\uFF0C\u8BF7\u624B\u52A8\u9009\u62E9\u590D\u5236';
          return ok;
        });
      }
    ...
    function saveFile(text, prefix, dbKey) {
      ...
      if (typeof window !== 'undefined' &&
          window.SQLDEV_BROWSER_UTILS &&
          typeof window.SQLDEV_BROWSER_UTILS.downloadSqlTextFile === 'function') {
        var savedFileName = window.SQLDEV_BROWSER_UTILS.downloadSqlTextFile(text, prefix, dbKey);
        if (savedFileName) statusText.value = '\u5DF2\u4FDD\u5B58\u4E3A ' + savedFileName;
        return;
      }
    ...
    function formatSqlText(text, preserveBlocks) {
      if (typeof window !== 'undefined' &&
          window.SQLDEV_SQL_UTILS &&
          typeof window.SQLDEV_SQL_UTILS.formatSqlText === 'function') {
        return window.SQLDEV_SQL_UTILS.formatSqlText(String(text || ''), { preserveBlocks: !!preserveBlocks });
      }
    ...
    async function _convertViaBackend(kind, input, fromDb, toDb) {
      ...
      function mapConvertErrorMessage(code, status) {
        if (typeof window !== 'undefined' &&
            window.SQLDEV_CONVERT_UTILS &&
            typeof window.SQLDEV_CONVERT_UTILS.mapConvertErrorMessage === 'function') {
          return window.SQLDEV_CONVERT_UTILS.mapConvertErrorMessage(code, status);
        }
        ...
        if (errCode === 'engine_init_failed') return '转换引擎加载失败，请重新部署 convert 函数后重试';
        if (errCode === 'conversion_failed') return '转换执行失败，请检查 SQL 内容后重试';
      ...
      if (result.error) {
        ...
        if (frontendRules && (status >= 500 || msgLower.indexOf('internal_error') >= 0 || msgLower.indexOf('conversion_failed') >= 0)) {
          try {
            result = await window.authApi.invokeFunction('convert', {
              kind: kind,
              input: input,
            ...
            if (!result.error) {
              var fallbackJson = result.data;
              if (fallbackJson && typeof fallbackJson.output === 'string') {
                _convertResultCache.set(cacheKey, fallbackJson.output);
