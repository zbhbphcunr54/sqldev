...
function isSupportedDatabase(value: unknown): value is SupportedDatabase {
...
export function convertDdlOrchestrated(
  ...
  const sourceDb = String(sourceDbValue || '').toLowerCase()
  const targetDb = String(targetDbValue || '').toLowerCase()

  if (!input.trim()) return '-- 请输入 DDL 语句'
  if (input.length > 5 * 1024 * 1024) return '-- 输入内容过大（超过 5MB），请分批处理'
  if (sourceDb === targetDb) return '-- 源数据库与目标数据库相同，无需转换'
  if (!isSupportedDatabase(sourceDb)) return `-- 不支持的源数据库: ${sourceDb}`
  if (!isSupportedDatabase(targetDb)) return `-- 不支持的目标数据库: ${targetDb}`
  ...
  if ((!tables || !tables.length) && !hasExtra && !hasViews) {
    return '-- 未识别到 CREATE TABLE / CREATE VIEW 语句，请检查输入格式'
  }
